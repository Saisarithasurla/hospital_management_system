...
import { Moon, Sun, Bell, Trash2, Check, CheckCheck, User, Calendar, FileText, Lock, LogOut, Menu } from "lucide-react";
...
function Navbar() {
  ...
  return (
    <div className="sticky top-0 left-0 z-30 bg-theme-secondary/95 shadow-theme-xl backdrop-blur-xl h-16 flex items-center justify-between px-4 md:px-8 border-b border-theme">
      <div className="flex items-center gap-1 md:gap-0">
        <button 
          onClick={() => setSidebarOpen(true)} 
          className="p-2 rounded-lg bg-theme-tertiary text-theme-primary hover:bg-theme-hover md:hidden flex items-center justify-center cursor-pointer" 
          title="Open Sidebar"
        >
          <Menu size={20} />
        </button>
      ...
      <div className="flex items-center gap-2 sm:gap-4">
        <div className="static sm:relative" ref={dropdownRef}>
          <button onClick={() => setIsOpen(!isOpen)} className="p-2 rounded-full bg-theme-tertiary text-theme-primary hover:bg-theme-hover transition-colors relative flex items-center justify-center cursor-pointer" title="Notifications">
            ...
            {unreadCount > 0 && (
              <span className="absolute -top-1 -right-1 inline-flex items-center justify-center px-1.5 py-0.5 text-[9px] font-bold leading-none text-red-100 bg-red-600 rounded-full animate-pulse">
                {unreadCount}
              </span>
            )}
          </button>
          {isOpen && (
            <div className="absolute right-4 sm:right-0 left-4 sm:left-auto mt-3 w-auto sm:w-96 bg-theme-card border border-theme rounded-xl shadow-theme-2xl z-50 overflow-hidden text-theme-primary">
              <div className="flex items-center justify-between px-4 py-3 border-b border-theme bg-theme-secondary/40">
                ...
                {unreadCount > 0 && (
                  <button onClick={markAllAsRead} className="text-2xs sm:text-xs text-blue-500 hover:text-blue-600 hover:underline flex items-center gap-1 font-medium transition-colors cursor-pointer">
                    <CheckCheck size={12} className="sm:w-3.5 sm:h-3.5" /> Mark all
                  </button>
                )}
              </div>
              <div className="max-h-60 sm:max-h-80 overflow-y-auto divide-y divide-theme">
                {notifications.length === 0 ? (
                  ...
                  notifications.map((n) => (
                    <div key={n.id} className={`p-3 sm:p-4 transition-colors flex gap-2 sm:gap-3 hover:bg-theme-hover relative group ${!n.is_read ? "bg-blue-500/5 border-l-4 border-blue-500 pl-2 sm:pl-3" : ""}`}>
                      <div className="shrink-0 mt-0.5">
                        {getIcon(n.notification_type)}
                      </div>
                      <div className="flex-1 pr-6">
                        <h4 className={`text-xs sm:text-sm ${!n.is_read ? "font-bold text-theme-primary" : "font-medium text-theme-secondary"}`}>
                          {n.title}
                        </h4>
                        <p className="text-[10px] sm:text-xs text-theme-muted mt-1 leading-relaxed">
                          {n.description}
                        </p>
                        <span className="text-[9px] sm:text-[10px] text-theme-muted block mt-1">
              ...
              {notifications.length > 0 && (
                <div className="px-4 py-2 bg-theme-secondary/40 border-t border-theme flex justify-end">
                  <button onClick={clearAllNotifications} className="text-2xs sm:text-xs text-red-500 hover:text-red-600 hover:underline flex items-center gap-1 font-medium transition-colors cursor-pointer">
        ...
        <button onClick={toggleTheme} className="p-2 rounded-full bg-theme-tertiary text-theme-primary hover:bg-theme-hover transition-colors cursor-pointer" title={`Switch to ${theme === "light" ? "dark" : "light"} mode`}>
        ...
        <div className="flex items-center gap-2 sm:gap-3 pl-2 border-l border-theme shrink-0">
          <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-blue-600 flex items-center justify-center text-white font-bold text-sm sm:text-base shrink-0">{firstLetter}</div>
          <div className="hidden md:block">
            <h3 className="font-semibold text-sm text-theme-primary leading-tight">{username}</h3>
            <p className="text-xs text-theme-muted">{roleDesc}</p>
          </div>
        </div>
      </div>
